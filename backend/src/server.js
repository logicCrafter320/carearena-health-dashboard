...
app.post("/api/patients/:id/vitals", async (req, res) => {
  ...
  const [result] = await pool.query(
    "INSERT INTO vitals (patient_id, systolic, diastolic, glucose, heart_rate, temperature) VALUES (?, ?, ?, ?, ?, ?)",
    [patientId, systolic, diastolic, glucose, heart_rate, temperature]
  );
  ...
  for (const [severity, title, message] of alerts) {
    await pool.query(
      "INSERT INTO alerts (patient_id, severity, title, message) VALUES (?, ?, ?, ?)",
      [patientId, severity, title, message]
    );
  }
...
app.post("/api/patients/:id/notes", async (req, res) => {
  ...
  const [result] = await pool.query(
    "INSERT INTO doctor_notes (patient_id, doctor_name, severity, note) VALUES (?, ?, ?, ?)",
    [req.params.id, doctor_name, severity, note]
  );
